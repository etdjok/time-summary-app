...
const changelogData: ChangeItem[] = [
  {
    date: '2026-08-21',
    version: 'v2.3.2',
    title: '移除默认密码 + 加密解锁引导 + 保存失败显式提示',
    changes: [
      '安全修复：彻底移除内置默认密码，全新部署首次启动强制设置登录密码（此前任何知道开源代码的人都可用默认密码登录局域网部署）',
      '新增：加密会话未解锁时，页面顶部显示醒目"解锁"横幅，避免输入内容后点发送无反应的困惑（加密密码不落盘，每个新会话需解锁一次属正常设计）',
      '修复：快速记录保存失败时显示明确错误原因并保留已输入内容，可在 F12 控制台查看具体失败环节',
      '修复：加密密码解锁/恢复码重置成功后自动重新拉取数据，云端备份状态显示不再误报"未备份"',
      '登录密码存储说明：服务端 password.json 仅保存 PBKDF2-SHA256 哈希（随机盐+10万次迭代），无明文；该文件已加入 .gitignore 禁止提交',
    ],
  },
  {
    date: '2026-08-20',
    version: 'v2.3.0',
    title: '每日打卡 + 桌面双栏布局 + 全局搜索',
    changes: [
      '新增每日打卡：把每天固定要做的事加为习惯，一键打卡、连续天数统计、月历回顾；数据加密存储于坚果云 _habits.json',
      '桌面端双栏布局：大屏幕下左侧固定录入+统计、右侧内容区，录入时不再被列表推走；手机端保持原有上下布局',
      '全局搜索：搜索框支持"当前周期/全部历史"范围切换，全库检索历史记录并按日期分组展示',
      '代码质量：修复解析器死分支与 4 处 React hooks 依赖告警，清理 30+ 处未使用导入/变量/any 类型，ESLint 全绿',
    ],
  },
  {
    date: '2026-08-20',
